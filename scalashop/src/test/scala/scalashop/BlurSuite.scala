...
class BlurSuite extends FunSuite {
  ...
  test("VerticalBoxBlur.blur with 32px wide image" +
    "should create 32 tasks") {
    val w = 32
    val h = 64
    val src = new Img(w, h)
    val dst = new Img(w, h)
        
   val v = VerticalBoxBlur.parBlur(src, dst, 32, 1)

    def check(x: Int, expected: Int) =
      assert(v == expected,
        s"($x tasks, should be $expected)")
        
    check(v, 32)  
}
    test("HBlur.blur with 32px wide image" +
    "should create 32 tasks") {
    val w = 64
    val h = 32
    val src = new Img(w, h)
   ...
   val v = HorizontalBoxBlur.parBlur(src, dst, 32, 1)
